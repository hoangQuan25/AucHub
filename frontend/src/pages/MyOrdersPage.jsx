    ...
    if (initialized && keycloak.authenticated) {
      const fetchOrders = async () => {
        ...
        try {
          ...
          const statusParamForApi = activeFilter === 'ALL' ? '' : `&status=${activeFilter}`;
          const response = await apiClient.get(`/orders/my?page=0&size=10${statusParamForApi}`);
    ...
    <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 bg-gray-100 min-h-screen">
      ...
      <div className="mb-6 bg-white shadow-sm rounded-md overflow-hidden">
        <nav className="flex flex-wrap border-b border-gray-200">
          {Object.entries(buyerOrderStatusFilters).map(([key, displayValue]) => (
            <button
              key={key}
            ...
            >
              {displayValue} {/* Display value from buyerOrderStatusFilters */}
            </button>
          ))}
        </nav>
      </div>
      ...
      {/* Order List */}
      {isLoading && <div className="text-center py-10">Đang tải đơn hàng...</div>}
      ...
      {!isLoading && !error && orders.length === 0 && (
         <div className="text-center py-10 bg-white rounded-md shadow-sm">
            <p className="text-gray-500">
                ...
                {activeFilter !== 'ALL' ? `cho trạng thái "${buyerOrderStatusFilters[activeFilter]}"` : ''}.
            </p>
         </div>
      )}
      {!isLoading && !error && orders.length > 0 && (
        <div className="space-y-4">
          {orders.map((order) => ( // order.status will be actual backend status key
            <div key={order.id} className="bg-white rounded-md shadow-sm border border-gray-200 overflow-hidden">
              <div className="px-4 py-3 border-b border-gray-200 bg-gray-50 flex justify-between items-center">
                <span className="font-semibold text-sm text-gray-700">
                  ...
                  Người bán: {order.sellerUsernameSnapshot || order.sellerName || 'N/A'}
                </span>
                <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${
                    // Example styling, can be more sophisticated
                    ['AWAITING_WINNER_PAYMENT', 'AWAITING_NEXT_BIDDER_PAYMENT'].includes(order.status)
                      ? 'bg-orange-100 text-orange-700'
                      : order.status === 'PAYMENT_SUCCESSFUL'
                      ? 'bg-green-100 text-green-700'
                      : ['AWAITING_FULFILLMENT_CONFIRMATION', 'AWAITING_SHIPMENT'].includes(order.status)
                      ? 'bg-blue-100 text-blue-700'
                       : order.status === 'COMPLETED'
                      ? 'bg-purple-100 text-purple-700'
                      : order.status === 'ORDER_SUPERSEDED_BY_REOPEN' // Buyer might see this
                ...
                >
                  {orderStatusMap[order.status] || order.status.replace(/_/g, ' ')} {/* Use master map */}
                </span>
              </div>
              ...
              <div className="px-4 py-3 bg-gray-50 flex flex-col sm:flex-row justify-end items-center gap-4">
                {(order.status === 'AWAITING_WINNER_PAYMENT' || order.status === 'AWAITING_NEXT_BIDDER_PAYMENT') && order.paymentDeadline && (
                  <div className="text-xs text-orange-600 font-medium flex items-center gap-1 mr-auto"> {/* Pushed to left */}
                ...
                <div className='text-right'>
                 ...
                 {(order.status === 'AWAITING_WINNER_PAYMENT' || order.status === 'AWAITING_NEXT_BIDDER_PAYMENT') && (
                    <Link to={`/orders/${order.id}/payment`}> {/* Example payment link */}
                      <button className="px-4 py-1.5 bg-red-600 text-white text-sm font-semibold rounded hover:bg-red-700 transition duration-150 ease-in-out shadow-sm">
                          Thanh toán
                      </button>
                    </Link>
                 )}
