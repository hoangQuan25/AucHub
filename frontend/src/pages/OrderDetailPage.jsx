...
const STRIPE_PUBLISHABLE_KEY = "pk_test_51RN788QoAglQPjjvhupJXkisXj7R7wt7epc8hYTUbDBTCxumwAownPBKNMM8NfNVza13yVVf6SrfAnmAxoiJtfRw00cIVf2LIl"; 
...
function OrderDetailPage() {
  ...
  const handleOpenPaymentAttempt = async () => {
    ...
    try {
      ...
      const response = await apiClient.post('/payments/create-intent', { // Path via gateway
        orderId: order.id,
        amount: order.currentAmountDue, // Assuming this is in smallest currency unit (e.g., Dong)
        currency: "vnd", // Your default currency
        userId: keycloak.subject,
        description: `Payment for Order #${order.id.substring(0,8)} - Auction: ${order.items[0]?.title || 'item'}`
      });
      ...
      setModalError(err.response?.data?.message || err.message || "Could not initiate payment.");
  ...
  const handleConfirmCancel = async () => {
    ...
    try {
      ...
      alert("Your request to cancel the payment for this order has been submitted.");
      ...
      setOrder(prev => ({ ...prev, status: 'CANCELLED' })); // Or a specific "buyer_cancelled" status
        ...
        setModalError(err.response?.data?.message || "Could not cancel this order payment. Please try again.");
  ...
  const item = order.items && order.items.length > 0 ? order.items[0] : { title: 'N/A', imageUrl: '/placeholder.png', variation: '', quantity: 0, price: 0 };
  const requiresPayment = order.status === 'PENDING_PAYMENT' || order.status === 'AWAITING_WINNER_PAYMENT' || order.status === 'AWAITING_NEXT_BIDDER_PAYMENT';
      ...
      <div className="max-w-md mx-auto p-4 sm:p-6 lg:p-8">
        ...
        <Elements stripe={stripePromise} options={options}>
          <CheckoutForm
            ...
            onSuccess={() => {
              alert("Payment submitted! Awaiting final confirmation from server.");
    ...
    <div className="max-w-4xl mx-auto p-4 sm:p-6 lg:p-8">
      ...
      <div className="bg-white rounded-lg shadow-md border border-gray-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200 bg-gray-50 flex justify-between items-center">
          <div>
            <span className="text-sm text-gray-600">Order for auction: {order.auctionId?.substring(0,8) || 'N/A'}</span>
            <p className="font-semibold text-gray-800">Seller: {order.sellerName || 'N/A'}</p>
          </div>
          <span className={`text-sm font-medium px-3 py-1 rounded-full ${
              requiresPayment ? 'bg-orange-100 text-orange-800' :
              order.status === 'DELIVERING' ? 'bg-blue-100 text-blue-800' :
              order.status === 'COMPLETED' ? 'bg-green-100 text-green-800' :
              order.status === 'CANCELLED' || order.status === 'ORDER_CANCELLED_BY_SELLER' || order.status === 'ORDER_CANCELLED_NO_PAYMENT_FINAL' ? 'bg-red-100 text-red-800' :
        ...
        <div className="p-6 flex flex-col sm:flex-row items-start gap-6">
            ...
            <div className="flex-grow">
                ...
                {item.variation && <p className="text-sm text-gray-600 mb-1">{item.variation}</p>}
                ...
                <p className="text-lg font-bold text-gray-800 mt-3">
                 ...
                 {(order.buyerPremium || (finalPrice - item.price) > 0) && ( // Show premium if exists or implied
                    <p className="text-sm text-gray-600">
                        + Buyer Premium: {(finalPrice - item.price).toLocaleString('vi-VN')} VNĐ
                    </p>
                 )}
            </div>
        </div>
        ...
        {requiresPayment && order.paymentDeadline && (
         ...
         {order.status === 'COMPLETED' && (
             <div className="px-6 py-4 border-t border-gray-200">
                <p className="text-sm text-green-700">Order completed. Thank you!</p>
             </div>
         )}
       ...
       <ConfirmationModal
         ...
         onClose={() => { if(!isProcessing) setIsPaymentConfirmOpen(false); }}
         onConfirm={handleConfirmPayment} // This now calls handleOpenPaymentAttempt
         title="Confirm Payment"
         message={`Proceed to payment for ${finalPrice.toLocaleString('vi-VN')} VNĐ?`}
