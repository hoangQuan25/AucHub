    ...
    const fetchProfile = async () => {
      if (initialized && keycloak.authenticated) {
        ...
        try {
          ...
          console.error(
            "useEffect: Failed during token update or profile fetch:",
            err
          );
          setProfileError(
        ...
        <>
          ...
          <div className="mb-6 p-4 border rounded bg-white shadow-sm">
            ...
            {profileData.paymentLast4Digits ? (
              <p>
                <strong>Card:</strong> {profileData.paymentCardType || "N/A"}{" "}
                ending in **** {profileData.paymentLast4Digits}
                <span className="text-gray-600">
                  {" "}
                  (Exp: {profileData.paymentExpiryMonth || "MM"}/
                  {profileData.paymentExpiryYear || "YYYY"})
                </span>
              </p>
            ) : (
