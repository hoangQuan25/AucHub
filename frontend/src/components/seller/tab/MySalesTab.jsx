// src/components/seller/tabs/MySalesTab.jsx
import React from 'react';
import { Link } from 'react-router-dom';
import { sellerOrderStatusFilters, orderStatusMap } from '../../../constants/orderConstants'; // Adjust path as needed
...
const MySalesTab = ({
  ...
  onOpenDecisionModal, // Renamed from handleOpenDecisionModal for clarity as prop
  // Add pagination props here if/when implemented for sales:
  // salesPage,
  // salesTotalPages,
  // onSalesPageChange,
  // salesPageSize = 10, // Default or from constant
}) => {
  // This component assumes it's only rendered when isOwner is true,
  // so no explicit isOwner check is needed inside for now,
  // unless specific parts within it should vary based on ownership (unlikely for "My Sales").

  return (
    <div className="bg-white p-4 sm:p-6 rounded-lg shadow-md">
      <div className="mb-6 border-b border-gray-200 -m-4 sm:-m-6">
        <nav className="flex flex-wrap px-4 sm:px-6">
          {Object.entries(sellerOrderStatusFilters).map(([key, value]) => (
            <button
              ...
              className={`py-3 px-4 sm:px-5 text-xs sm:text-sm font-medium focus:outline-none transition-colors duration-150 ease-in-out ${
      ...
      {isLoadingSales && <div className="text-center py-10">Đang tải đơn bán...</div>}
      {!isLoadingSales && salesError && (
        <div className="text-center py-10 text-red-600">{salesError}</div>
      )}
      ...
      {!isLoadingSales && !salesError && salesOrders.length > 0 && (
        <div className="space-y-4">
          {salesOrders.map((order) => (
            <div key={order.id} className="bg-white rounded-md shadow-sm border border-gray-200 overflow-hidden">
              ...
              <div className="px-4 py-3 bg-gray-50 flex flex-col sm:flex-row justify-between items-center gap-3 border-t">
                ...
                {order.status === 'AWAITING_SHIPMENT' && (
                  <button className="px-4 py-1.5 bg-blue-500 text-white text-sm font-semibold rounded-md hover:bg-blue-600 transition duration-150 ease-in-out shadow-sm">
          ...
          {/* TODO: Add PaginationControls for sales orders if salesTotalPages > 1 */}
        </div>
      )}
    </div>
  );
