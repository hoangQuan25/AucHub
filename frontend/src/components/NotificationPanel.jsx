    ...
    >
      ...
      <div className="overflow-y-auto flex-grow">
        {notifications.length === 0 ? (
          <p className="text-center text-gray-500 text-sm p-6">No recent notifications.</p>
        ) : (
          <ul className="divide-y divide-gray-100">
            {/* Only show maybe the latest 10-15 in the dropdown? Or all received? */}
            {notifications.slice(0, 15).map((notif) => ( // Limit displayed items
              <li
                ...
                className={`p-3 text-sm hover:bg-gray-100 transition-colors duration-100 ${!notif.isRead ? 'bg-blue-50 font-medium cursor-pointer' : 'text-gray-600 cursor-default'}`}
              >
                <p className={`leading-relaxed ${!notif.isRead ? 'text-gray-900' : 'text-gray-700'}`}>{notif.message}</p>
                <p className="text-xs text-gray-400 mt-1">
                  {new Date(notif.timestamp).toLocaleString()} {/* Use timestamp field */}
                </p>
              </li>
            ))}
          </ul>
       ...
       <div className="p-2 text-center border-t bg-gray-50">
            <button onClick={handleViewAllClick} className="text-sm text-indigo-600 hover:underline font-medium">
